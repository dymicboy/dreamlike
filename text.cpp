...
static const char*	vert_text_path = "../dreamlike.vert";
static const char*	frag_text_path = "../dreamlike.frag";
...
void render_text( std::string text, GLint _x, GLint _y, GLfloat scale, vec4 color, GLfloat dpi_scale )
{
	...
	glUniform1i(glGetUniformLocation(program_text, "use_color"), true);
	glUniform4f(glGetUniformLocation(program_text, "color"), color.r, color.g, color.b, color.a);
	...
	for (c = text.begin(); c != text.end(); c++)
	{
		...
		glUniformMatrix4fv(glGetUniformLocation(program_text, "model_matrix"), 1, GL_TRUE, text_matrix);
